...
export function Header() {
  ...
  const [mobileDumpsterOpen, setMobileDumpsterOpen] = useState(false);
  ...
  const dumpsterSubmenu = [
    {
      href: "/dumpsters/rubber-wheeled-dumpsters",
      label: "Rubber Wheeled Dumpsters",
    },
    {
      href: "/dumpsters/roll-off-dumpsters",
      label: "Roll off Dumpsters",
    },
    {
      href: "/dumpsters/permanent-dumpsters",
      label: "Permanent Dumpsters",
    },
  ];
  ...
  const isActive = (path: string) => {
    ...
    if (path.startsWith("/#"))
      return pathname === "/" && activeHash === path.replace("/", "");
                ...
                if (link.submenu) {
                  return (
                    <div
                      key={link.href}
                      className="relative"
                      onMouseEnter={() => setShowDumpsterMenu(true)}
                      onMouseLeave={() => setShowDumpsterMenu(false)}
                    >
                      <Link
                        href={link.href}
                        className={cn(
                          "flex items-center gap-1 text-sm font-semibold py-5 border-b-2 transition-all duration-200",
                          pathname.startsWith("/dumpsters")
                      ...
                      {showDumpsterMenu && (
                        <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 z-50">
                          <div className="w-[320px] rounded-2xl border border-white/10 bg-[#0B1730]/95 backdrop-blur-2xl shadow-[0_20px_60px_rgba(0,0,0,0.45)] overflow-hidden">

                            {/* Header */}
                            <div className="px-5 pt-4 pb-2 border-b border-white/5">
                              <p className="text-xs font-semibold tracking-widest uppercase text-[#FFD700]">
                                Dumpster Services
                              </p>
                            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {isLoggedIn && user ? (
                ...
                <Button
                  ...
                  className="bg-[#FFD700] text-slate-900 hover:bg-yellow-400"
                  size="sm"
                >
                  Sign In
                </Button>
              ...
              <Link href="/cart">
                ...
                >
                  ...
                  <span className="absolute -top-1 -right-1 bg-white text-slate-900 text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold">
                    2
                  </span>
                </Button>
              </Link>
                ...
                if (link.submenu) {
                  return (
                    <div key={link.href} className="space-y-2">
                      <button
                        onClick={() =>
                          setMobileDumpsterOpen(!mobileDumpsterOpen)
                        }
                        className={cn(
                      ...
                      >
                        <Link href={link.href}>Dumpsters</Link>

                        <ChevronDown
                          size={18}
                          className={cn(
                            "transition-transform duration-200",
                            mobileDumpsterOpen && "rotate-180"
                          )}
                        />
                      </button>
                          ...
                          {link.submenu.map((sub) => (
                            <Link
                              ...
                              className={cn(
                                ...
                                pathname === sub.href
                                  ? "bg-[#FFD700] text-slate-900 font-semibold"
