...
export function BookingWidget() {
  ...
  useEffect(() => {
    ...
    const fetchPrice = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(errorMsg || "Pricing unavailable");
        ...
        if (msg.includes("Service is not currently available") || msg.includes("coming soon")) {
          setZipError("Service is not currently available in your area");
          setPriceError("");
        } else {
          setPriceError(msg);
        }
  ...
  const showStep34 = isStep1Done && isStep2Done;
  const showStep56 = isStep1Done && isStep2Done && isStep3Done && isStep4Done;
      ...
      <div className="max-w-4xl scale-[0.95] mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-10 text-left border-4 border-[#C89B2B]">
          ...
          <div className="mb-6 relative">
            ...
            {zipError && (
              <div className="mt-3 flex items-start gap-3 p-3.5 rounded-xl border-2 border-red-200 bg-red-50 text-red-950 shadow-sm animate-in fade-in slide-in-from-top-1 duration-200">
                <AlertCircle className="h-5 w-5 text-red-600 shrink-0 mt-0.5" />
                <div>
                  <h4 className="font-bold text-sm text-red-900 leading-tight">Service Unavailable</h4>
                  <p className="text-xs font-semibold text-red-700 mt-1 leading-relaxed">{zipError}</p>
                </div>
              </div>
            )}
