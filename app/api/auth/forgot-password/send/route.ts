...
export async function POST(req: Request) {
  try {
    ...
    // Note: Supabase JS doesn't have a direct getUserByEmail without elevated privileges
    ...
    if (!userExists) {
      // Return success anyway to prevent email enumeration, but don't actually send email
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; text-align: center;">
          ...
          <p>We received a request to reset your password for your Blue Sky Disposal account. Please use the following 6-digit verification code to proceed:</p>
          <div style="background-color: #f4f4f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <h1 style="color: #2563eb; letter-spacing: 4px; margin: 0; font-size: 32px;">${otp}</h1>
          </div>
          <p style="color: #64748b; font-size: 14px;">This code will expire in 10 minutes. If you did not request a password reset, please ignore this email or contact support if you have concerns.</p>
        </div>
      `,
    };
