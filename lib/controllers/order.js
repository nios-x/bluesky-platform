...
export async function getOrderByIdController(supabase, user, orderId) {
  // 1️⃣ Get customer linked to this user
  const { data: customer, error: custError } = await getCustomerByUserId(
    supabase,
    user.id,
  );
  ...
  const { data: order, error: orderError } = await getOrderDetails(
    supabase,
    orderId,
  );
  ...
  // 3️⃣ Security check → ensure the order belongs to this customer
...
export async function getMyOrdersController(supabase, user) {
  // 1️⃣ Get customer linked with this authenticated user
  const { data: customer, error: custError } = await getCustomerByUserId(
    supabase,
    user.id,
  );
  ...
  // 2️⃣ Fetch all orders for this customer
  const { data: orders, error: ordersError } = await getCustomerOrders(
    supabase,
    customer.id,
  );
