...
export async function processOrderAndSaveToDB(bookingsData, contactInfo, paymentDetails) {
  ...
  const { data: billingAddress, error: billingError } = await supabaseAdmin
    ...
    .insert({
      ...
      address: firstBooking.address || "Pending Address",
      city: firstBooking.city || "Pending City",
      state: firstBooking.state || "MI",
      zip: firstBooking.zipCode || "00000"
    })
    .select()
    .single();
  ...
  const { data: serviceAddress, error: serviceAddressError } = await supabaseAdmin
    ...
    .insert({
      customer_id: customer.id,
      address_line1: firstBooking.address || "Pending Address",
      city: firstBooking.city || "Pending City",
      state: firstBooking.state || "MI",
      zip: firstBooking.zipCode || "00000",
      is_default: true
  ...
  for (const bData of bookingsList) {
    ...
    serviceInserts.push({
      ...
      delivery_date: bData.deliveryDate || new Date().toISOString().split('T')[0],
      pickup_date: new Date(new Date().setDate(new Date().getDate() + (bData.rentalPeriod || 7))).toISOString().split('T')[0],
      service_address_id: serviceAddress.id,
      price: bData.totalPrice || (paymentDetails.amount / bookingsList.length),
      weight_capacity_tons: bData.dumpsterCapacity || null
    });
  }
